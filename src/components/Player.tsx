...
const Player = () => {
    ...
    const {data, refetch} = useQuery(isFavoriteQuery, {
      variables: {userId: 'gus', trackId: track?.id || ''},
  });
  const isLiked = data?.favoritesByTrackidAndUserid?.length > 0;
    ...
    const onLike = async () => {
      if(!track) return;
      if(isLiked){
        await removeFavorite ({variables: { userId: 'gus', trackId: track.id}});
      }else {
        await insertFavorite({variables: { userId: 'gus', trackId: track.id}});
      }
